...
func (obj BGPPolicyConditionConfig) CreateDBTable(dbHdl *sql.DB) error {
	dbCmd := "CREATE TABLE IF NOT EXISTS BGPPolicyConditionConfig " +
		"( " +
		"Name TEXT, " +
		"ConditionType TEXT, " +
		"IpPrefix TEXT, " +
...
func (obj BGPPolicyConditionConfig) DeleteObjectFromDb(objKey string, dbHdl *sql.DB) error {
	sqlKey, err := obj.GetSqlKeyStr(objKey)
	if err != nil {
		fmt.Println("GetSqlKeyStr for BGPPolicyConditionConfig with key", objKey, "failed with error", err)
		return err
	}

	dbCmd := "delete from BGPPolicyConditionConfig where " + sqlKey
	fmt.Println("### DB Deleting BGPPolicyConditionConfig\n")
	_, err = dbutils.ExecuteSQLStmt(dbCmd, dbHdl)
	return err
}

func (obj BGPPolicyConditionConfig) GetObjectFromDb(objKey string, dbHdl *sql.DB) (ConfigObj, error) {
...
func (obj BGPPolicyConditionConfig) GetAllObjFromDb(dbHdl *sql.DB) (objList []ConfigObj, err error) {
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("DB method Query failed for 'BGPPolicyConditionConfig' with error BGPPolicyConditionConfig", dbCmd, err))
		return objList, err
	}
...
func (obj BGPPolicyConditionConfig) MergeDbAndConfigObj(dbObj ConfigObj, attrSet []bool) (ConfigObj, error) {
	var mergedBGPPolicyConditionConfig BGPPolicyConditionConfig
	objTyp := reflect.TypeOf(obj)
	objVal := reflect.ValueOf(obj)
	dbObjVal := reflect.ValueOf(dbObj)
	mergedObjVal := reflect.ValueOf(&mergedBGPPolicyConditionConfig)
	idx := 0
	for i := 0; i < objTyp.NumField(); i++ {
	...
	return mergedBGPPolicyConditionConfig, nil
}

func (obj BGPPolicyConditionConfig) UpdateObjectInDb(dbObj ConfigObj, attrSet []bool, dbHdl *sql.DB) error {
	var fieldSqlStr string
	dbBGPPolicyConditionConfig := dbObj.(BGPPolicyConditionConfig)
	objKey, err := dbBGPPolicyConditionConfig.GetKey()
	objSqlKey, err := dbBGPPolicyConditionConfig.GetSqlKeyStr(objKey)
	dbCmd := "update " + "BGPPolicyConditionConfig" + " set"
	objTyp := reflect.TypeOf(obj)
	objVal := reflect.ValueOf(obj)
	idx := 0
	for i := 0; i < objTyp.NumField(); i++ {
