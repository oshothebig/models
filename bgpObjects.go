...
type SourcePolicyList struct {
	Sources string `"SNAPROUTE: "KEY", ACCESS:"w", MULTIPLICITY:"*". DESCRIPTION: Source Protocol(s) which BGP is interested in. Multiple sources can be specified as comma separated strings when the same policy needs to be applied", SELECTION:"CONNECTED"/"STATIC"/"OSPF"`
