...
func (obj BGPPolicyActionConfig) CreateDBTable(dbHdl *sql.DB) error {
	dbCmd := "CREATE TABLE IF NOT EXISTS BGPPolicyActionConfig " +
		"( " +
		"Name TEXT, " +
		"ActionType TEXT, " +
...
func (obj BGPPolicyActionConfig) DeleteObjectFromDb(objKey string, dbHdl *sql.DB) error {
	sqlKey, err := obj.GetSqlKeyStr(objKey)
	if err != nil {
		fmt.Println("GetSqlKeyStr for BGPPolicyActionConfig with key", objKey, "failed with error", err)
		return err
	}

	dbCmd := "delete from BGPPolicyActionConfig where " + sqlKey
	fmt.Println("### DB Deleting BGPPolicyActionConfig\n")
	_, err = dbutils.ExecuteSQLStmt(dbCmd, dbHdl)
	return err
}

func (obj BGPPolicyActionConfig) GetObjectFromDb(objKey string, dbHdl *sql.DB) (ConfigObj, error) {
...
func (obj BGPPolicyActionConfig) GetKey() (string, error) {
	key := string(obj.Name)
	return key, nil
}

func (obj BGPPolicyActionConfig) GetSqlKeyStr(objKey string) (string, error) {
	keys := strings.Split(objKey, "#")
	sqlKey := "Name = " + "\"" + keys[0] + "\""
	return sqlKey, nil
}

func (obj *BGPPolicyActionConfig) GetAllObjFromDb(dbHdl *sql.DB) (objList []*BGPPolicyActionConfig, e error) {
	dbCmd := "select * from BGPPolicyActionConfig"
	rows, err := dbHdl.Query(dbCmd)
	if err != nil {
		fmt.Println(fmt.Sprintf("DB method Query failed for 'BGPPolicyActionConfig' with error BGPPolicyActionConfig", dbCmd, err))
		return objList, err
	}
	...
	for rows.Next() {
		...
		object := new(BGPPolicyActionConfig)
...
func (obj BGPPolicyActionConfig) MergeDbAndConfigObj(dbObj ConfigObj, attrSet []bool) (ConfigObj, error) {
	var mergedBGPPolicyActionConfig BGPPolicyActionConfig
	objTyp := reflect.TypeOf(obj)
	objVal := reflect.ValueOf(obj)
	dbObjVal := reflect.ValueOf(dbObj)
	mergedObjVal := reflect.ValueOf(&mergedBGPPolicyActionConfig)
	idx := 0
	for i := 0; i < objTyp.NumField(); i++ {
	...
	return mergedBGPPolicyActionConfig, nil
}

func (obj BGPPolicyActionConfig) UpdateObjectInDb(dbObj ConfigObj, attrSet []bool, dbHdl *sql.DB) error {
	var fieldSqlStr string
	dbBGPPolicyActionConfig := dbObj.(BGPPolicyActionConfig)
	objKey, err := dbBGPPolicyActionConfig.GetKey()
	objSqlKey, err := dbBGPPolicyActionConfig.GetSqlKeyStr(objKey)
	dbCmd := "update " + "BGPPolicyActionConfig" + " set"
	objTyp := reflect.TypeOf(obj)
	objVal := reflect.ValueOf(obj)
	idx := 0
	for i := 0; i < objTyp.NumField(); i++ {
